...
-- Code Desciption:
---------------------------------------------------------------------------
-- PostgreSQL/PostGIS custom function for generating a lattice structure comprising equilateral triangles of a given radius (triangle side length). The function uses a single or collection of geometries to define the grid extents.
-- Dependencies: DE_MakeHexagon(), DE_MakeTriHexagon()
-- Developed by: mark[at]dimensionaledge[dot]com
-- Licence: GNU GPL version 3.0
---------------------------------------------------------------------------
-- Usage Example:
---------------------------------------------------------------------------
-- DROP TABLE IF EXISTS triangular_grid;
-- CREATE TABLE triangular_grid AS (
...
-- s AS (SELECT DE_TriangularGrid(ST_Envelope(ST_Collect(r.extent_geom)),10) as wkb_geometry FROM r)
-- SELECT row_number() over() as gid, s.wkb_geometry FROM s);
---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION DE_TriangularGrid(envelope GEOMETRY, radius FLOAT8)
...
WHILE y_value  + 2*vstep > YMIN LOOP  -- for each y value, reset x to XMIN and subloop through the x values
	...
	WHILE x_value - radius < XMAX LOOP
		...
		hexagon := ST_SnapToGrid(DE_MakeHexagon(centerpoint, radius),0.000001);
		x_count := x_count + 1; 
		x_value := x_value + hstep;
		RETURN QUERY SELECT ST_SnapToGrid(DE_MakeTriHexagon(hexagon),0.000001);
